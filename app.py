# Need to pull expiration and pair with description

from selenium import webdriver
from bs4 import BeautifulSoup
import time

loggingFile = open("logging.txt", "w")
loggingFile.write("\nstart of logging\n")
edgeDriver = webdriver.Edge('C:\Windows\WinSxS\wow64_microsoft-webdriver-server-components_31bf3856ad364e35_10.0.17763.1_none_edeaf67448991e72\MicrosoftWebDriver.exe')
time.sleep(3)
edgeDriver.get("https://www.heb.com/digital-coupon/coupon-selection")
time.sleep(5)
hebClosePopup = edgeDriver.find_elements_by_class_name("close-reveal-modal")
for i in range(len(hebClosePopup)):
    if hebClosePopup[i].is_displayed():
        edgeDriver.execute_script("arguments[0].click();", hebClosePopup[i])
        time.sleep(1)
lenOfPage = edgeDriver.execute_script("window.scrollTo(0, document.body.scrollHeight);var lenOfPage=document.body.scrollHeight;return lenOfPage;")
match=False
while(match==False):
    lastCount = lenOfPage
    time.sleep(3)
    ...
    if lastCount==lenOfPage:
        match=True
heb_page_source = edgeDriver.page_source
heb_soup = BeautifulSoup(heb_page_source,features='lxml')
heb_description=[]
heb_description_selector= heb_soup.find_all('div', class_='coupon-description')
for i in heb_description_selector:
    target = str(i.get_text())
    heb_description.append(target[11:])
print(heb_description)




# UNDER is searchbar functions
try:
...
if __name__ == "__main__":
    ...
    def command(text): #search command response
        print("remove me")

    root = Tk()
    SearchBox(root, command=command, placeholder="Product Name", entry_highlightthickness=0).pack(pady=6, padx=3)
